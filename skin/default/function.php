...
function showPlugin() {
	...
	if (!$obj->debrid_mode) {
		...
		$acc = $obj->acc[$obj->debrid_host]['accounts'];
		if (empty($acc[0]) == false)
			if ($obj->hide_number_acc == true)
				echo '<span class="plugincollst">' . $obj->debrid_host . '</span>';
			else
				echo '<span class="plugincollst">' . $obj->debrid_host . ' ' . count($acc) . '</span>';
	}
	return false;
}
?>