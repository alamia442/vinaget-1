...
if (!empty($_POST["accounts"])) {
	...
	if (check_account($type, $account)) 
		die("false duplicate");
	if ($obj->debrid_mode) {
		require_once('hosts/debrid/' . $obj->list_host_debrid[$type]['file']);
		$download = new $obj->list_host_debrid[$type]['class']($obj, $type);
	}
	else {
	...
	if (method_exists($download, "CheckAcc")) {
		if (strpos($account, ":")) {
			if (method_exists($download, "Login")) {
				list($user, $pass) = explode(':',$account);
				$cookie = $download->Login($user, $pass);
			}
			else {
				echo "false only support cookie";
				$update = false;
			}
		}
		else $cookie = $account;
