<?php
$config = array();
$config['secure'] = true;
$config['password'] = "abcxyz!@#$%";
$config['admin'] = "xyzabc!@#$%";
$config['hide_page'] = false;
$config['keyword_page'] = "vnz";
$config['homepage'] = "";
$config['download_prefix'] = "";
$config['download_suffix'] = "";
$config['limitMBIP'] = 100 * 1024;
$config['ttl'] = 6 * 60;
$config['limitPERIP'] = 10;
$config['ttl_ip'] = 1;
$config['max_jobs_per_ip'] = 100;
$config['max_jobs'] = 500;
$config['max_load'] = 50;
$config['max_size_default'] = 100 * 1024;
$config['file_size_limit'] = 100 * 1024;
$config['title'] = "[color=red]»ᵭoωŋŁoαd«[/color]";
$config['colorfilename'] = "green";
$config['colorfilesize'] = "red";
$config['listfile'] = true;
$config['showlinkdown'] = true;
$config['showdirect'] = true;
$config['redirectdl'] = false;
$config['bbcode'] = true;
$config['privatefile'] = false;
$config['privateip'] = false;
$config['checkacc'] = true;
$config['checklinksex'] = false;
$config['rename'] = true;
$config['delete'] = true;
$config['display_error'] = true;
$config['adslink'] = false;
$config['api_ads'] = "";
$config['tinyurl'] = false;
$config['badword'] = "porn, sex, hentai, xxx, jav";
$config['language'] = "english";
$config['skin'] = "default";
$config['hide_plugins_col'] = false;
$config['hide_preacc_col'] = false;
$config['hide_number_acc'] = false;
$config['del_checked_acc'] = false;
$config['debrid_mode'] = false;
$config['debrid_host'] = 'real-debrid.com';
?>